...
vamapping virtual_memory_lookup(x86_pagetable* pagetable, uintptr_t va) {
    ...
    // get the page antry located at this virtual address!
    // how does a page entry relate to the virtual address? 
    x86_pageentry_t pe = pagetable->entry[L1PAGEINDEX(va)];
    
    // if this page entry is real...
    if (pe & PTE_P) {
    
        // get page table based off of page entry. 
        x86_pagetable* l2pagetable = (x86_pagetable*) PTE_ADDR(pe);
        
        // get the page entry of the level 2 page table; do something with flags
        pe = l2pagetable->entry[L2PAGEINDEX(va)] & (pe | ~(PTE_W | PTE_U));
    }
    // if page exists, return something... but what? 
    if (pe & PTE_P)
...
void set_pagetable(x86_pagetable* pagetable) {
    ...
    // arg1: pagetable
    // arg2: default_int_handler (262320);
