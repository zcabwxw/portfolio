...
$(document).ready(function () {
    ...
    /*
     * Let's declare some global objects & variables. 
     */
    ...
    /*tooltiprect.setOffset({
		
		y: tooltiprect.getHeight()/2
		});*/
    ...
    function setDimensions(nWidth, nHeight) {
        ...
        // resize the stage and container height, but don't make it too small!
        /*  if (nHeight > 550) {
            stage.setHeight(nHeight + 50);

            $('#container').css('height', nHeight + 50);
        } else {
            stage.setHeight(550);
            $('#container').css('height', 550);
        }
		
		//reset globals so we have better drag bounds
...
function changeWall()
{
	var thisWall = stage.get('#wall0')[0];
	/*thisWall.transitionTo({
		points: [150, 150, 600, 150, 600, 200, 150, 200],
		duration: 1
		})
	thisWall.setPoints([150, 150, 600, 150, 600, 200, 150, 200]);
	boardLayer.draw();
}*/
    ...
    function captureImage() {
		...
		{
			dataURL(mySpace, myPanel);
		}
	
    }
	
	
	function dataURL(mySpace, myPanel)
	{
			setTimeout(function () {
			showMyTools(mySpace, myPanel)
			}, 2000);
...
function rotateImage(number, myDiff) {
    ...
    // var toolbarButtons = [shapeFrames, imageFrames, textFrames, eraseFrames, rotateFrames, resizeFrames];
    ...
    /*
     *  Tracks current position, increments moveCounter
     */
    ...
    /*function addMessageText(myText, myX, myY, myID) {
	...
	/*	 var canvas = document.getElementById('container');
      ...
      context.fillText('Hello World!', x, y);
		...
		messageText.setOffset(messageText.getWidth()/2, messageText.getHeight()/2);
    ...
    // creates the object palette with the notes
    function objectPalette(myLayer, myColor) {
	...
	{
		// //// // // // // console.log('rect-anchors');
		for (e = 0; e < 4; e++) // only four sides to a rectangle 
        {
			// //// // // // // console.log('building anchor-'+e);
            var xIndex = e * 2;
            var yIndex = e * 2 + 1;
    ...
    function fixedDrag(anchor, m, b, radius) {
        ...
        anchor.setDragBoundFunc(function (pos) {
         ...
         //   $('#instructions').text('m: ' + m);

            // keep anchor on the line. 

            newY = (lineY == pos.y) ? newY : lineY;

            //if (anchor.attrs.polyType=='reg')
    ...
    function rotateDrag(anchor, radius, myShape) {
        ...
        anchor.setDragBoundFunc(function (pos) {
            ...
            //$('#instructions').text('radius: ' + Math.round(radius) + ", cdist: " + Math.round(cDist) + " coord: ("+Math.round(pos.x)+","+Math.round(pos.y) + ")");
    ...
    function moveAnchor(anchor, radius, newX, newY) {
        ...
        /*	// //// // // // // console.log("id: " + anchor.attrs.id);
		// //// // // // // console.log("numbers: " + posX +","+posY+","+newX+","+newY+","+cx+","+cy);*/
		...
		// //// // // // // console.log("anchorDistance: " + anchorDist);
...
1) Figure out upload image function with PHP

2) Add shapes onto different groups within the artLayer.
